  ...
  const handleOpenPdfPreview = async () => {
    ...
    try {
      ...
      const builtFormData = {
        ...
        medications: (senior.medicines || []).map((m: any) => typeof m === 'string' ? m : m.name || ''),
        livingArrangement: senior.livingWith?.join(', ') || '',
        estimatedMonthlyIncome: senior.monthlyIncomeRange || '',
        receivingPension: !!senior.monthlyPension && senior.monthlyPension !== 'None',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            title="Preview & Export NCSC Data Form (PDF)"
            className="flex items-center justify-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all duration-150 active:scale-95 cursor-pointer"
          >
            <FileDown size={14} />

            <span className="hidden sm:inline">NCSC PDF</span>
          </button>
        </div>
            ...
            {ncscData ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-[11px] font-medium text-slate-600 leading-normal">
                ...
                <div className="space-y-3.5">
                  ...
                  <div>
                    <span className="text-[8.5px] text-slate-400 uppercase tracking-wider block">
                      Receiving Social Pension
                    </span>

                    <p className="text-slate-800 font-bold uppercase">
                      {ncscData.receivingSocialPension ? "Yes" : "No"}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p
                      className={`font-bold font-mono uppercase text-[10px] ${ncscData.status === "Completed" ? "text-teal-600" : ncscData.status === "Pending" ? "text-amber-600" : "text-slate-600"}`}
                    >
                      {ncscData.status}
                    </p>
                  </div>
