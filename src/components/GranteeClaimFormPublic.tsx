...
export default function GranteeClaimFormPublic({ onBack }: Props) {
  ...
  const [oscaPassword, setOscaPassword] = useState('');
  ...
  const handleSubmit = async () => {
    ...
    // Validate OSCA Number against seniors table and centenarian_honoring table

    if (!oscaNumber) { showToast('Please enter your LGU OSCA ID Number', 'error'); return; }
    if (!oscaPassword) { showToast('Please enter your password', 'error'); return; }

    // 1. Check seniors table — must be "Qualified for Honoring"
    const { data: seniorRecord, error: seniorError } = await supabase
      .from('seniors')
      .select('status, password')
      .eq('osca_number', oscaNumber)
      .maybeSingle();

    if (seniorError) { showToast('Failed to verify OSCA Number. Please try again.', 'error'); return; }

    if (!seniorRecord || seniorRecord.password !== oscaPassword) {
      showToast('Invalid OSCA ID or password. Please try again.', 'error');
      return;
    }

    if (!seniorRecord || seniorRecord.status !== 'Qualified for Honoring') {
      setShowNotQualifiedModal(true);
      return;
    }

    // 2. Check centenarian_honoring — latest record must NOT be Rejected or soft-deleted
    const { data: honoringRecord, error: honoringError } = await supabase
    ...
    if (honoringError) { showToast('Failed to verify honoring record. Please try again.', 'error'); return; }

    if (honoringRecord && (honoringRecord.status === 'Rejected' || honoringRecord.deleted_at !== null)) {
      setShowNotQualifiedModal(true);
      return;
    }
  ...
  const renderStep1 = () => (
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <InputField label="C.3 Password" value={oscaPassword} onChange={setOscaPassword} placeholder="Enter your password" type="password" />
